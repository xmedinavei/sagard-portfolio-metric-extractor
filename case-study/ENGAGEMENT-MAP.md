...
> `interest_margin` and refused beside SaaS margins; ClearPay cash `38.4M` shown net of `$6.2M` client float =
> `32.2M`). **P4 = the D5 cross-validation flag** (MediSight own `27.9M` vs summary `22.4M` surfaced with delta
